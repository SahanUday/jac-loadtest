...
def test_services_map_json_parsed(monkeypatch):
    ...
    monkeypatch.setattr("jac_loadtest.bridge.topology._load_toml_routes", lambda: {})
    config = _cfg(
        mode="microservice",
        services_map='{"order_service": "http://order:18001"}',
    )
    router = TopologyRouter.from_config(config)
    url, svc = router.resolve("http://rec:8000/order_service/foo")
...
def test_longest_prefix_wins():
    ...
    router = TopologyRouter(routes=routes)
...
def test_shorter_prefix_no_false_match():
    ...
    router = TopologyRouter(routes=routes, fallback_url="http://fallback:9")
    url, svc = router.resolve("http://rec:8000/walker/order/create")
    assert svc == "gateway"
...
def test_no_match_routes_to_gateway():
    ...
    router = TopologyRouter(routes=routes, fallback_url="http://gateway:8000")
...
def test_jac_toml_discovery(monkeypatch):
    monkeypatch.setattr(
        "jac_loadtest.bridge.topology._load_toml_routes",
        lambda: {
...
def test_no_fallback_and_no_match_raises():
    routes = [ServiceRoute(name="orders", prefix="/walker/order", url="http://order:1")]
    router = TopologyRouter(routes=routes, fallback_url=None)
